...
// ✦ - ✦ - ✦ - ✦ - ✦ - ✦ - ✦ - ✦ - ✦ - ✦
// 🔓 19. РОЗБЛОКУВАННЯ КОДУ (Fill-in-the-blanks)
// ✦ - ✦ - ✦ - ✦ - ✦ - ✦ - ✦ - ✦ - ✦ - ✦
document.addEventListener("DOMContentLoaded", () => {
  console.log("🚀 Скрипт розблокування коду запущено!"); // Діагностика

  const blanks = document.querySelectorAll(".blank");
  const pageId =
    window.location.pathname.replace(/[^a-zA-Z0-9]/g, "_") || "default_page";

  // 1. АВТОГЕНЕРАЦІЯ ІНПУТІВ
  blanks.forEach((blank, index) => {
    const answer = blank.getAttribute("data-answer");
    const storageKey = "wu_blank_" + pageId + "_" + index;

    blank.className = "inline-sizer";
    blank.setAttribute("data-value", "");

    const input = document.createElement("input");
    input.type = "text";
    input.className = "unlock-input";
    input.setAttribute("data-answer", answer); // Надійне збереження відповіді
    input.setAttribute("data-key", storageKey);
    input.placeholder = "?";
    ...
    if (savedValue) {
      ...
      blank.setAttribute("data-value", savedValue);
    }

    blank.appendChild(input);
  });

  // Функція очищення (Абсолютно безпечна до кодування файлу завдяки Unicode)
  const cleanAnswer = (text) => {
    ...
    return (
      text
        .toLowerCase()
        .trim()
        .replace(/\s+/g, "") // Видаляємо випадкові пробіли
        .replace(/\u0441/g, "c")
        .replace(/\u043E/g, "o")
        .replace(/\u0430/g, "a")
        .replace(/\u0435/g, "e")
        .replace(/\u0456/g, "i")
        // Залишаємо ТІЛЬКИ англійські літери, цифри та український алфавіт (через Unicode)
        .replace(/[^a-z0-9\u0430-\u044F\u0456\u0457\u0454\u0491]/g, "")
    );
  };

  // 2. ФУНКЦІЯ ПЕРЕВІРКИ ТА РОЗБЛОКУВАННЯ
  const checkTask = (input) => {
    const expected = cleanAnswer(input.getAttribute("data-answer"));
    const current = cleanAnswer(input.value);

    // Вивід у консоль, щоб бачити, що саме порівнює браузер
    console.log(
      `[Перевірка] Очікується: "${expected}" | Введено: "${current}"`,
    );

    // Підсвічуємо поточний інпут
    if (current === expected && current !== "") {
      input.classList.add("success");
      input.style.borderBottomColor = "mediumseagreen"; // Примусовий зелений колір
      input.style.color = "mediumseagreen";
    } else {
    ...
    // Перевіряємо всі інпути в межах поточного завдання
    const taskContainer = input.closest(".unlockable-task");
    if (taskContainer) {
      const allInputs = Array.from(
        taskContainer.querySelectorAll(".unlock-input"),
      );
      const codeWrapper = taskContainer.querySelector(".locked-code-wrapper");

      const allCorrect = allInputs.every((inp) => {
        const exp = cleanAnswer(inp.getAttribute("data-answer"));
        const cur = cleanAnswer(inp.value);
        return cur === exp && cur !== "";
      });

      // Розблоковуємо фінальний код
      if (codeWrapper) {
        if (allCorrect) {
          codeWrapper.classList.add("unlocked");
        } else {
          codeWrapper.classList.remove("unlocked");
        }
      }
    }
  };
  ...
  document.addEventListener("input", (e) => {
    if (!e.target.classList.contains("unlock-input")) return;

    const input = e.target;
    const blank = input.closest(".inline-sizer");

    if (blank) {
      blank.setAttribute("data-value", input.value);
    }
  ...
  // 4. ПЕРВИННА ПЕРЕВІРКА ПІСЛЯ ЗАВАНТАЖЕННЯ
  setTimeout(() => {
    document.querySelectorAll(".unlock-input").forEach((inp) => {
      if (inp.value) checkTask(inp);
    });
  }, 100);
});
